...
function validateConfig() {
	console.log('[deploy2] 배포 설정 검증 중...');
...
function testSSHConnection() {
	...
	try {
		cp.execSync(`ssh -i "${SSH_KEY}" -o StrictHostKeyChecking=no -o ConnectTimeout=10 ${VERIFY_USER}@${VERIFY_SERVER} "echo 'SSH 연결 성공'"`, {
...
function uploadRelease(release) {
	...
	try {
		...
		cp.execSync(`ssh -i "${SSH_KEY}" -o StrictHostKeyChecking=no ${VERIFY_USER}@${VERIFY_SERVER} "rm -rf ${remotePath} && mkdir -p ${remotePath}"`, {
			stdio: 'inherit'
		});

		// 릴리스 폴더 전체 업로드
		cp.execSync(`scp -i "${SSH_KEY}" -o StrictHostKeyChecking=no -r "${release.path}"/* ${VERIFY_USER}@${VERIFY_SERVER}:${remotePath}/`, {
			stdio: 'inherit'
		});
...
function installPackage(remotePath) {
	...
	try {
		cp.execSync(`ssh -i "${SSH_KEY}" -o StrictHostKeyChecking=no ${VERIFY_USER}@${VERIFY_SERVER} '${script}'`, {
			stdio: 'inherit'
		});
...
function verifyDeployment() {
	...
	try {
		// 포트 80 (Nginx) 확인
		const port80Result = cp.execSync(`ssh -i "${SSH_KEY}" -o StrictHostKeyChecking=no ${VERIFY_USER}@${VERIFY_SERVER} "curl -s -o /dev/null -w '%{http_code}' http://localhost || echo 'CURL_FAILED'"`, {
		...
		try {
			const externalTest80 = cp.execSync(`curl -s -o /dev/null -w '%{http_code}' --connect-timeout 10 http://${VERIFY_SERVER} || echo 'EXTERNAL_FAILED'`, {
				stdio: ['ignore', 'pipe', 'ignore'],
				encoding: 'utf8',
				timeout: 15000
			}).trim();
			...
			if (externalTest80 === '200' || externalTest80 === '302') {
				...
				console.warn('   GCP 콘솔에서 방화벽 규칙에 포트 80을 허용하세요');
			} else {
...
// 메인 함수
function main() {
	console.log('🚀 NanumPay 검증 서버 배포 시작');
	console.log('==============================');
