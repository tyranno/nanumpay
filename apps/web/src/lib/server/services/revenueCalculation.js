...
export async function calculateMonthlyRevenue(year, month, skipExistingCheck = false) {
  ...
  // F1 지급액 (회당)
  const f1Total = revenuePerInstallment * gradeRatios.F1;
  const f1Divisor = gradeCount.F1 + gradeCount.F2;
  gradePayments.F1 = f1Divisor > 0 ? f1Total / f1Divisor : 0;
  
  // F2 지급액 (회당)
  const f2Total = revenuePerInstallment * gradeRatios.F2;
  const f2Divisor = gradeCount.F2 + gradeCount.F3;
  gradePayments.F2 = f2Divisor > 0 ? (f2Total / f2Divisor) + gradePayments.F1 : gradePayments.F1;
  
  // F3 지급액 (회당)
  const f3Total = revenuePerInstallment * gradeRatios.F3;
  const f3Divisor = gradeCount.F3 + gradeCount.F4;
  gradePayments.F3 = f3Divisor > 0 ? (f3Total / f3Divisor) + gradePayments.F2 : gradePayments.F2;
  
  // F4 지급액 (회당)
  const f4Total = revenuePerInstallment * gradeRatios.F4;
  const f4Divisor = gradeCount.F4 + gradeCount.F5;
  gradePayments.F4 = f4Divisor > 0 ? (f4Total / f4Divisor) + gradePayments.F3 : gradePayments.F3;
  
  // F5 지급액 (회당)
  const f5Total = revenuePerInstallment * gradeRatios.F5;
  const f5Divisor = gradeCount.F5 + gradeCount.F6;
  gradePayments.F5 = f5Divisor > 0 ? (f5Total / f5Divisor) + gradePayments.F4 : gradePayments.F4;
  
  // F6 지급액 (회당)
  const f6Total = revenuePerInstallment * gradeRatios.F6;
  const f6Divisor = gradeCount.F6 + gradeCount.F7;
  gradePayments.F6 = f6Divisor > 0 ? (f6Total / f6Divisor) + gradePayments.F5 : gradePayments.F5;
  
  // F7 지급액 (회당)
  const f7Total = revenuePerInstallment * gradeRatios.F7;
  const f7Divisor = gradeCount.F7 + gradeCount.F8;
  gradePayments.F7 = f7Divisor > 0 ? (f7Total / f7Divisor) + gradePayments.F6 : gradePayments.F6;
  
  // F8 지급액 (회당)
  const f8Total = revenuePerInstallment * gradeRatios.F8;
  const f8Divisor = gradeCount.F8;
  gradePayments.F8 = f8Divisor > 0 ? (f8Total / f8Divisor) + gradePayments.F7 : gradePayments.F7;
