<script>
	...
	function getFridaysInMonth(year, month) {
		...
		return fridays.length; // 해당 월의 금요일 개수 (주차 수)
	}
								...
								{#each ['F1', 'F2', 'F3', 'F4', 'F5', 'F6', 'F7', 'F8'] as grade}
									<tr class="data-row">
										...
										{#each periodColumns as column}
											...
											<td class="data-col">
												<span class="text-xs"
													>{gradeData.amount.toLocaleString()}({gradeData.count})</span
												>
											</td>
										{/each}
									</tr>
									...
									{#each periodColumns as column}
										...
										<td class="data-col">
											<span class="text-xs font-bold"
												>{totalAmount.toLocaleString()}({totalCount})</span
											>
										</td>
									{/each}
								</tr>
