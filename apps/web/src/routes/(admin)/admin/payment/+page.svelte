<script>
	...
	function updateDisplayData() {
		...
		if (periodType === 'monthly') {
			...
			console.log(`Displaying months ${displayStartIndex + 1} to ${endIndex} of ${monthlyData.length}`);
		} else {
			// 주간 표시: 기존 방식대로
			...
			console.log(`Displaying weeks ${displayStartIndex + 1} to ${endIndex} of ${allWeeklyData.length}`);
		}

		console.log('Display columns:', weeklyColumns.map(w => w.label));
		processPaymentData();
	}
	...
	function handlePeriodChange() {
		...
		if (startYear !== prevStartYear || startMonth !== prevStartMonth ||
			endYear !== prevEndYear || endMonth !== prevEndMonth) {
			// 기간이 변경되면 새로 로드
			allWeeklyData = [];
			lastLoadedStartYear = startYear;
			lastLoadedStartMonth = startMonth;
		...
		<div class="filter-box">
			<div class="filter-content single-line">
				<label class="radio-label">
					<input type="radio" bind:group={filterType} value="date" onchange={handleFilterTypeChange} />
					<span>주간</span>
				</label>
				<input
					type="date"
				...
				<label class="radio-label">
					<input type="radio" bind:group={filterType} value="period" onchange={handleFilterTypeChange} />
					<span>기간</span>
				</label>
				{#if filterType === 'period'}
					<input
	...
	<div class="search-section">
		...
		<div class="page-settings">
			<label class="per-page-label">
				페이지당
				<select bind:value={itemsPerPage} onchange={handleItemsPerPageChange} class="per-page-select">
	...
	{:else}
						...
						{#each getCurrentPageData() as user}
							<tr>
								...
								{#each weeklyColumns as week}
									...
									<td class="amount-cell" title={payment?.installmentDetails ? payment.installmentDetails.map(d => `${d.revenueMonth} ${d.installmentNumber}회차`).join(', ') : ''}>
		...
		{#if filteredPaymentList.length > 0}
			<div class="pagination">
				...
				<div class="page-info">
					총 {filteredPaymentList.length}명 중 {(currentPage - 1) * itemsPerPage + 1}-{Math.min(currentPage * itemsPerPage, filteredPaymentList.length)}명
				</div>
			</div>
		{/if}
		</div>
...
<style>
	...
	.year-input, .month-input, .week-input {
		padding: 5px;
		border: 1px solid #ccc;
		border-radius: 3px;
		font-size: 14px;
