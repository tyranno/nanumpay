...
async function processFridayPayments() {
  ...
  // === 2단계: 10회 완료 확인 및 추가 계획 생성 ⭐ 신규 ===
  const completedPlans = plans.filter(p => p.completedInstallments === 10);
  ...
  // === 3단계: 승급자 확인 ===
  const gradeChanges = await checkGradeChanges();
  for (const change of gradeChanges) {
    if (change.promoted) {
      await createPromotionPaymentPlan(change.userId, change.newGrade, today);
    }
  }
