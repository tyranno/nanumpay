...
def create_excel(filename, data_list, registration_month):
    ...
    # 저장 경로 설정
    save_dir = os.path.join(os.path.dirname(__file__), '..', 'test-data', 'test')
...
def main():
    ...
    print('📊 데이터 정의 확인:')
    print(f'  7월: {july_simple_count}명 (ID: {len(JULY_SIMPLE)}개)')
    ...
    print('\n📊 전체 요약:')
    print(f'  7월: 3명 (기존 파일 사용)')
    print(f'  8월: {len(august_data)}명 (누적 {3 + len(august_data)}명)')
    print(f'  9월: {len(september_data)}명 (누적 {3 + len(august_data) + len(september_data)}명)')
    print(f'  10월: {len(october_data)}명 (누적 {3 + len(august_data) + len(september_data) + len(october_data)}명)')
    print('\n✅ 모든 테스트 데이터 생성 완료!')
